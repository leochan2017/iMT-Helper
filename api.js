...
const imtHelperAxios = axios.create()
...
function sendBarkMsg (msg) {
  ...
  return barkAxios({
    method: 'get',
    url: `${bark}/${encodeURIComponent('iMT-Helper')}/${encodeURIComponent(`账号${cookies.indexOf(currentCookie) + 1} ` + msg)}?group=${encodeURIComponent('iMT-Helper')}`
  })
}
  ...
  return await imtHelperAxios({
    ...
    .catch(async d => {
      ...
      if (d.response.status === 401 && d.response.data) {
        await sendBarkMsg(`Login fail, Please update cookie setting`)
